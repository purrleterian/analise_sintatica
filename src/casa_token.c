...
void CasaToken(const char *expected) {
  ...
  if (!t || strcmp(t->type, TOKEN_EOF) == 0) {
...
void VarPart() {
  CasaToken("KW_VAR");
  printf("<parte de declaracoes de variaveis> ::= { var <declaracao de "
...
void VarDecl() {
  printf("<declaracao de variaveis> ::= <lista de identificadores> : <tipo>\n");
  printf(
...
void Statement() {
  ...
  printf("<comando> ::= <atribuicao> | <comando composto> | <comando "
